...
export const approve = async (walletClient: any, client: any, tokenAddress: Hex, contractAddress: Hex, amount: bigint, logger: any) => {
    ...
    if (allowance < amount) {
        ...
        logger.info(`${walletClient.account.address} | Success approve: https://arbiscan.io/tx/${txHash}`)

        const sleepTime = random(sleepAfterApprove[0], sleepAfterApprove[1])
        logger.info(`${walletClient.account.address} | Waiting ${sleepTime} sec after approve before deposit...`)
